...
class DecisionServiceTest {
    ...
    void thresholdBoundaries(double score, String expected) {
        DecisionService service = new DecisionService(
            Mockito.mock(ScoreRepository.class), Mockito.mock(DecisionRepository.class), 0.8, 0.4);
        assertEquals(expected, service.decide(score));
    }
    ...
    void recordPersistsScoreAndDecision() {
        ...
        DecisionService service = new DecisionService(scores, decisions, 0.8, 0.4);
        Session session = newSession();

        var response = service.record(
            session, new MlServiceClient.ScoreDto(0.87, "human", "v1", List.of()));
